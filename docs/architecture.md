...
管理员自由探测由 Admin 解析账号与代理选择、审计动作。HTTP 模式由 Provider 提供账号认证报头，Host 通过注入的 HTTP 探测端口发送请求，原始交换不进入状态缓存。WebSocket 预热由 OpenAI Provider 在所选出口发送官方 `generate=false` 准备请求，合格 State 写入该账号和模型的进程内缓存。两种模式都不进入普通请求观测或探测报头持久化，也不参与 State 自动续采。
...
管理端从账号实时模型目录选择模型，手动诊断与自动续采均向 Provider 提供完整的已保存代理目录及数据库中的探测策略，不使用直连。
手动探测与自动续采开关独立保存，关闭后不启动对应的新轮次，已有轮次使用准入时的策略快照完成。
Store 在设置事务内保存策略、验证代理引用并写入审计，代理删除使用同一设置行锁防止悬空引用。
Provider 统一负责候选选择、探测调度与状态提交：同一账号的手动探测和自动续采跨模型串行，
相邻实际探测请求至少间隔 10 秒，每个账号滚动 60 秒内最多发送 3 次；单实例 OpenAI Provider 最多同时持有 2 个活跃探测请求。
每轮手动与自动均按已保存策略最多处理 1～3 个不同候选，固定代理仅处理 1 个，取得有效 state 后立即停止。
连接配置失败占用候选预算，但不产生实际请求记录；等待发送间隔不占用候选名额。
没有实际请求的轮次不写入探测历史。滚动预算在下一次发送前耗尽时结束本轮，不持有账号锁等待下一个窗口。

智能策略依次使用账号当前绑定代理、当前账号/模型的成功首选、账号级成功偏好，再轮换其余候选。
固定策略只允许指定代理，代理池策略仅在选中目录项内轮换，随机策略对全部或选中目录中可用候选无放回抽样。
所有策略共用冷却与账号预算，策略筛选不删除未选中代理的调度记录。
网络失败、超时、代理认证失败及连接配置失败的冷却归属于账号与代理组合；
312、缺少有效 state 及其他上游错误的冷却归属于账号、模型与代理组合，后续轮次按对应作用域跳过失败候选。
账号认证拒绝与账号限流终止当前轮次，不记作代理故障；账号限流另由账号级退避约束。
键级失败退避只在实际发送过请求、未成功提交 state 且状态版本仍匹配时更新。

state、探测历史、成功偏好、轮换位置、冷却及退避均属于 Provider 的进程内运行态，进程重启后丢失。
管理视图只读聚合脱敏状态，不发起上游请求；核心 state 缓存不返回管理端，也不写 PostgreSQL、Redis、审计或日志。
HTTP/SSE 取得上游响应或 WebSocket `response.create` 成功写入后才确认 state 已应用；
读取缓存、构造请求和 WebSocket 握手不改变应用状态。确认时核对账号、模型与 state 原文，
旧业务请求不能标记已轮换的新值。

探测提交受账号运行代次、凭据版本、本轮候选身份和状态版本约束。
业务响应刷新或撤销状态后，旧探测不得覆盖新状态，也不得追加旧轮次的键级失败退避；
实际请求事实仍可保留在历史中。候选身份同时包含代理标识和连接配置：
每轮准入时同步当前候选集合，移除已删除或配置已变更候选的旧冷却与成功偏好，旧配置的调度状态不沿用到新配置。
运行中的探测使用本轮候选快照，不把代理目录变更视作对在途请求的即时取消。

探测报头采集是独立的短期诊断边界，默认关闭且只在本进程的限时窗口内接收手动探测与自动续采事实，
不采集普通 Responses 流量。Provider 只在窗口开启时复制 reqwest/hyper 可见的原始请求头和响应头，
通过有界 `try_send` 交给 Store 单写入 Worker 批量落 PostgreSQL；缓冲区或数据库故障只丢失诊断记录，
不得阻塞探测或改变其结果。记录保留 24 小时，过期后停止查询并由 retention Worker 删除。
列表只返回摘要，详情默认遮盖认证、Cookie、API Key、token、secret、credential 与 turn state 等敏感值；
显式显示原文需要管理员审计。内置逻辑备份排除这些记录的数据，避免恢复延长其敏感生命周期。

每个实例独立运行每 10 秒检查、无 leader 租约的续采 Worker，由 Provider 提供受账号忙碌、发送间隔、冷却与退避约束的待续采键。
最近 1 小时内实际发送过业务请求的账号/模型键具有自动续采资格，不依赖业务是否携带 State；空缓存、TTL 剩余 5 分钟、过期或失效时可以续采。
业务活跃记录与 State 应用时间分别维护，替换 State 不清除业务活跃记录，手动和自动探测都不延长活跃期。
重启、新账号、新模型与闲置后的首个业务请求都可重新登记待采集键，不依赖上游再次下发 State。
Worker 在执行前重新检查该键是否仍需续采，Provider 在候选处理及实际发送前继续核对，
避免排队期间业务已刷新状态后继续发出无效探测。
...
避免重新授权期间展示混合身份信息；单项失败不丢弃另一项可用结果。前端账号列表按当前页有界并发读取
订阅与重置卡摘要，短期复用查询结果，离开页面后取消个人信息请求并丢弃排队任务；打开「个人信息」或
手动刷新时仍可读取最新信息。重置卡次数复用既有浏览器会话状态，消费结果同步反映在列表中。
