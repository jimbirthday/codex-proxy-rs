...
管理端从账号实时模型目录选择模型，手动诊断向 Provider 提供完整的已保存代理候选集合，自动续采提供账号当前出口快照，支持直连与未保存的代理。
Provider 统一负责候选选择、探测调度与状态提交：同一账号的手动探测和自动续采跨模型串行，
相邻实际探测请求至少间隔 10 秒，每个账号滚动 5 分钟内最多发送 3 次；单实例 OpenAI Provider 最多同时持有 2 个活跃探测请求。
每轮手动最多处理 3 个不同候选，自动最多处理 1 个，取得有效 state 后立即停止。
连接配置失败占用候选预算，但不产生实际请求记录；没有实际请求的轮次不写入探测历史。

候选选择依次使用账号当前绑定代理、当前账号/模型的成功首选、账号级成功偏好，再轮换其余候选，并过滤冷却中的出口。
...
每个实例独立运行无 leader 租约的续采 Worker，由 Provider 提供受账号忙碌、发送间隔、冷却与退避约束的待续采键。
